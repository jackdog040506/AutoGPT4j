...
public interface OpsTicketsJpaRepository extends JpaRepository<OpsTickets, String> {
	...
	@Query("select ot from OpsTickets ot left join fetch ot.tasks ott where ot.ticketsTemplateId = ?1 ")
	...
	@Query("select ot from OpsTickets ot left join fetch ot.tasks ott where ot.id = ?1 ")
